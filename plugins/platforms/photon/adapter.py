...
# the iMessage stream can wedge after a burst of stream interruption/failure
# logs: local /healthz may stay green and outbound sends can still work, but no
# inbound messages reach Hermes. Restart only the Node sidecar after a short
# burst of these upstream warnings; the Python /inbound loop reconnects to the
# replacement sidecar with the same token, avoiding a full gateway restart.
_SIDECAR_STREAM_RESET_WINDOW_SECONDS = 120.0
_SIDECAR_STREAM_RESET_THRESHOLD = 4
_SIDECAR_STREAM_RESET_PATTERNS = (
    "stream interrupted; reconnecting",
    "stream persistently failing",
    "RST_STREAM",
    "SubscribeMessageEvents INTERNAL",
    "SubscribePollEvents INTERNAL",
)
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    async def _dispatch_inbound(self, event: Dict[str, Any]) -> None:
        ...
        if ctype == "text":
            ...
            # iMessage emits an object-replacement placeholder (U+FFFC) just
            # before some attachment/voice events. Treating that as a real text
            # turn makes the gateway answer garbage and can mark the actual
            # media event as an interruption. Drop placeholder-only messages at
            # the adapter boundary; the following attachment/voice event carries
            # the useful payload.
            if text.strip() == "\ufffc":
                logger.debug("[photon] dropping placeholder-only text event")
                return
